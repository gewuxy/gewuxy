...
public class UserAction extends BaseAction implements ModelDriven<Student>{
	...
	public String logon() throws Exception{
		...
		if(studentDao.login(student.getEmail(), student.getPassword())!=null){
			session.put("user", studentDao.login(student.getEmail(), student.getPassword()));//将学生放入user 的session
		...
		else if(parentDao.login(student.getEmail(), student.getPassword())!=null){
			session.put("user", parentDao.login(student.getEmail(), student.getPassword()));//将学生放入user 的session
		...
		else if(teacherDao.login(student.getEmail(), student.getPassword())!=null){
			session.put("user", teacherDao.login(student.getEmail(), student.getPassword()));//将学生放入user 的session
		...
		else return ERROR;//锟斤拷锟斤拷锟斤拷站锟斤拷页
	}
	/**
	 * 锟矫伙拷锟剿筹拷
	 * @return String
